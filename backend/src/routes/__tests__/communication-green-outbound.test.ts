...
describe('POST /api/communication/whatsapp — Green outbound CRM thread', () => {
  ...
  it('still attempts fallback send when fallback state is temporarily unknown but status is online', async () => {
    global.fetch = vi.fn(async (url: string, init?: RequestInit) => {
      if (url.includes('/waInstance7103104732/getStateInstance/')) {
        return { ok: true, status: 200, json: async () => ({ stateInstance: 'yellowCard' }) };
      }
      ...
      return {
        ok: true,
        status: 200,
        json: async () => ({ idMessage: 'fallback-green-state-unknown' }),
      };
    }) as any;
    ...
    await request(app)
      ...
      .send({
        phone: '052-123-4567',
        message: 'בודק fallback state unknown',
        customerId: 'customer-1',
        customerName: 'ישראל ישראלי',
      })
      .expect(200);
    ...
    expect(global.fetch).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        method: 'POST',
        body: JSON.stringify({ chatId: '972521234567@c.us', message: 'בודק fallback state unknown' }),
      }),
    );
    expect(prisma.waMessage.create).toHaveBeenCalledWith({
      data: expect.objectContaining({
        waMessageId: 'green:fallback-green-state-unknown',
      }),
    });
  });
