...
import { authenticate, cycleRosterOrAdmin, managerOrAdmin } from '../middleware/auth.js';
...
// Update registration
registrationsRouter.put('/:id', cycleRosterOrAdmin, async (req, res, next) => {
  try {
    const id = uuidSchema.parse(req.params.id);
...
// Delete registration
registrationsRouter.delete('/:id', cycleRosterOrAdmin, async (req, res, next) => {
  try {
    const id = uuidSchema.parse(req.params.id);
...
// Cancel registration
registrationsRouter.post('/:id/cancel', cycleRosterOrAdmin, async (req, res, next) => {
  try {
    const id = uuidSchema.parse(req.params.id);
...
registrationsRouter.post('/:id/send-cancellation-form', cycleRosterOrAdmin, async (req, res, next) => {
  try {
    const id = uuidSchema.parse(req.params.id);
