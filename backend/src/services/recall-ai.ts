...
interface ScheduleCycleOptions {
  cycleId: string;
  joinEarlyMinutes?: number;
}

export function isGoogleMeetUrl(url: string | null | undefined) {
  if (!url) return false;
  try {
...
export async function scheduleRecallBotForMeeting(options: ScheduleBotOptions) {
  ...
  if (!isGoogleMeetUrl(meeting.zoomJoinUrl)) {
    throw new Error('Recall lesson reports are currently enabled only for Google Meet links');
  }
...
export async function scheduleRecallBotsForCycle(options: ScheduleCycleOptions) {
  ...
  const scheduled: Array<{ meetingId: string; botId: string; joinAt?: string }> = [];
  const skipped: Array<{ meetingId: string; reason: string }> = [];

  for (const meeting of meetings) {
    if (!isGoogleMeetUrl(meeting.zoomJoinUrl)) {
      skipped.push({ meetingId: meeting.id, reason: 'not_google_meet' });
      continue;
    }
    ...
    try {
      const bot = await scheduleRecallBotForMeeting({
        meetingId: meeting.id,
        joinEarlyMinutes: options.joinEarlyMinutes,
      });
      scheduled.push({ meetingId: meeting.id, botId: bot.id, joinAt: bot.join_at });
    } catch (error: any) {
      skipped.push({ meetingId: meeting.id, reason: error.message || 'schedule_failed' });
    }
  }
