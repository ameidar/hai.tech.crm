...
export const useCreateSystemUser = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (data: { email: string; name: string; phone?: string; role: 'admin' | 'manager' | 'sales' | 'operations' | 'operations_control'; password?: string; hourlyRate?: number | null; city?: string | null; bankName?: string | null; bankBranch?: string | null; accountNumber?: string | null }) =>
...
export const useUpdateSystemUser = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ id, data }: { id: string; data: { name?: string; phone?: string; role?: 'admin' | 'manager' | 'sales' | 'operations' | 'operations_control'; isActive?: boolean; hourlyRate?: number | null; city?: string | null; bankName?: string | null; bankBranch?: string | null; accountNumber?: string | null } }) =>
