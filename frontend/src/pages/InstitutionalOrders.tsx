...
import { FileText, Building2, Search, Filter, ChevronUp, ChevronDown, ChevronsUpDown, LayoutGrid, List, Phone, RefreshCcw, Plus, Pencil, Trash2, CheckSquare, Square, X, Upload, Loader2 } from 'lucide-react';
...
export default function InstitutionalOrders() {
  ...
  const orderForm = (
    <OrderForm
      ...
      onCancel={() => { setShowAddModal(false); setEditItem(null); setFormError(null); }}
    />
  );
        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className={`${viewMode === 'grid' ? 'grid' : 'hidden'} grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-4`}>
              {filtered.map((order) => (
                <div key={order.id} onClick={() => navigate(`/institutional-orders/${order.id}`)} className={`group cursor-pointer bg-white rounded-xl border shadow-sm hover:shadow-lg transition-all duration-200 hover:-translate-y-0.5 p-5 ${selectedIds.has(order.id) ? 'border-blue-400 ring-2 ring-blue-100' : 'border-gray-100 hover:border-gray-200'}`}>
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <button onClick={e => { e.stopPropagation(); openEdit(order); }} className="p-1 text-gray-400 hover:text-blue-600 transition-colors opacity-0 group-hover:opacity-100"><Pencil size={14} /></button>
                      <button onClick={e => { e.stopPropagation(); setDeleteItem(order); }} className="p-1 text-gray-400 hover:text-red-500 transition-colors opacity-0 group-hover:opacity-100"><Trash2 size={14} /></button>
                    </div>
                  </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {filtered.map((order) => (
                      <tr key={order.id} onClick={() => navigate(`/institutional-orders/${order.id}`)} className={`hover:bg-gray-50 transition-colors group cursor-pointer ${selectedIds.has(order.id) ? 'bg-blue-50' : ''}`}>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button onClick={e => { e.stopPropagation(); openEdit(order); }} className="p-1 text-gray-400 hover:text-blue-600 transition-colors" title="ערוך"><Pencil size={14} /></button>
                            <button onClick={e => { e.stopPropagation(); setDeleteItem(order); }} className="p-1 text-gray-400 hover:text-red-500 transition-colors" title="מחק"><Trash2 size={14} /></button>
                          </div>
                        </td>
                      </tr>
                    ))}
